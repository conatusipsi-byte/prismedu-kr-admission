...
export function norm(s: string): string {
  return s
    ...
    // 불릿/마커 글리프(요강 각주표식)도 흡수 — ◆■●○□◈▪▫ (기존 ★◇◉☆▲△ 의 흑백 변종). 학과명 의미 무관.
    .replace(/[★◇◉◆■●○□◈▪▫*※☆▲△・·•∙ㆍᆢ‧⋅･․．\s]/g, "")
    .replace(/[()（）]/g, "")
...
export const NAME_ALIAS: Record<string, Record<string, string>> = {
  ...
  // ── Phase 2 batch 7.5 (2026-06-21) — 7차 미매칭 재정합. 요강 명칭변경 표 확정 1:1 rename만 alias(추정 금지). inje=계열 명칭변경(2026→2027) 3건, dhu=소방안전환경→소방안전, u1=문화복지융합 학과→학부. ──
  kcue_0000061: { "소방안전학과": "소방안전환경학과" }, // 대구한의대 batch 7.5 (명칭변경 표)
  kcue_0000164: { "의생명보건⋅의료공학계열": "의생명보건계열", "공학·건축계열": "공학계열", "컴퓨터·AI⋅게임⋅스마트물류계열": "컴퓨터·AI계열" }, // 인제대 batch 7.5 (계열 명칭변경 표)
  kcue_0000154: { "문화복지융합학부": "문화복지융합학과" }, // 유원대 batch 7.5 (학과→학부)
};
